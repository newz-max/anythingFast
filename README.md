...
- 计划触发无人值守执行
- 动作序列配置和流程预览
- 支持打开程序、URL、文件、文件夹、执行命令、运行脚本、延时等待、剪贴板读写、系统通知和 TCP 端口等待
- 按顺序执行动作
- 执行失败提示和摘要日志
- 高风险动作二次确认
- 本地 JSON 持久化配置
...
| 文档 | 内容 |
| --- | --- |
| [快速上手](./docs/快速上手.md) | 核心概念、主窗口、快捷搜索面板和创建第一个事项的流程 |
| [动作配置](./docs/动作配置.md) | 动作类型、系统集成动作、执行顺序、条件、超时、失败后继续和流程预览 |
| [运行变量](./docs/运行变量.md) | 变量 Key、默认值、必填、敏感变量、`{{ variableKey }}` 引用和动作输出绑定 |
| [触发器与快捷键](./docs/触发器与快捷键.md) | 手动触发、全局唤起、事项快捷键、软件内快捷键和周期触发 |
| [执行、风险与日志](./docs/执行、风险与日志.md) | 手动/无人值守执行、高风险确认、命令输出和执行日志 |
| [导入、导出与模板](./docs/导入、导出与模板.md) | JSON 导入导出、模板、导入预览、ID 冲突和路径提示 |
...
- 高风险命令、首次执行命令事项和读取剪贴板事项必须二次确认；读取剪贴板不能通过事项快捷键或计划触发静默执行。
